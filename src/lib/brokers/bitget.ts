...
// ──────────────────────────────────────────────────────────────────────
// Signed POST + write helpers (Phase 2: real orders)
// ──────────────────────────────────────────────────────────────────────
...
async function signedPost<T>(
  ...
  // Bitget signs the EXACT body bytes we send. Stringify once, sign that
  // string, then send the same string.
  const bodyStr = JSON.stringify(body);
  const timestamp = Date.now().toString();
  const signature = sign(timestamp, "POST", path, bodyStr, creds.secret);
...
export async function fetchContractSpec(
  ...
  const res = await fetch(
    `${BASE}/api/v2/mix/market/contracts?${qs}`,
  ...
  const c = json.data.find((x) => x.symbol === symbol) ?? json.data[0];
  if (!c) return null;

  // Bitget V2 quirk: `priceEndStep` is a multiplier, NOT the actual tick.
  // Real tick = priceEndStep × 10^(-pricePlace). Example: BTCUSDT returns
  // priceEndStep=1 + pricePlace=1 → tick 0.1 USDT.
  const safe = (v: string | undefined | null, fallback = 0): number => {
    const n = Number(v);
    return Number.isFinite(n) && n > 0 ? n : fallback;
  };
  const pricePlace = safe(c.pricePlace, 1);
  const priceMultiplier = safe(c.priceEndStep, 1);
  const realPriceTick = priceMultiplier / Math.pow(10, pricePlace);

  return {
    symbol: c.symbol,
    baseCoin: c.baseCoin,
    quoteCoin: c.quoteCoin,
    sizeMultiplier: safe(c.sizeMultiplier, 1),
    priceEndStep: realPriceTick,
    minTradeNum: safe(c.minTradeNum, 0),
    minTradeUSDT: safe(c.minTradeUSDT, 5),
    // Bitget sometimes names this `keepMarginRate`; default to 0.5% if absent.
    maintainMarginRate: safe(c.maintainMarginRate ?? c.keepMarginRate, 0.005),
    symbolStatus: c.symbolStatus ?? "normal",
  };
}

/**
 * Position mode of the user's USDT-FUTURES account.
 * "one_way_mode" — single direction per symbol; tradeSide MUST be omitted.
 * "hedge_mode"   — long+short can coexist; tradeSide REQUIRED.
 * Auto-place only supports one-way.
 */
export async function getPositionMode(
  creds: BitgetCreds,
): Promise<"one_way_mode" | "hedge_mode" | "unknown"> {
  type R = { posMode: string };
  const data = await signedGet<R>(
    creds,
    "/api/v2/mix/account/account",
    { productType: "USDT-FUTURES", symbol: "BTCUSDT", marginCoin: "USDT" },
  );
  if (data.posMode === "one_way_mode" || data.posMode === "hedge_mode") {
    return data.posMode;
  }
  return "unknown";
}
...
/**
 * Single open position for (symbol, marginCoin). Used to skip setLeverage
 * when a position is already open (Bitget rejects with 40914 otherwise).
 */
export async function getSinglePosition(
  ...
  const rows = await signedGet<R>(
    creds,
    "/api/v2/mix/position/single-position",
    { symbol, productType: "USDT-FUTURES", marginCoin: "USDT" },
  );
  const live = rows.find((r) => Number(r.total) > 0);
  if (!live) {
    return { hasPosition: false, leverage: null, side: null, size: 0 };
  }
  return {
    hasPosition: true,
    leverage: Number(live.leverage),
    side: live.holdSide,
    size: Number(live.total),
  };
}

/**
 * Switch account position mode between one-way and hedge.
 * Bitget rejects this if any position is open or any pending order exists
 * on the productType — the error code surfaces in BitgetError.toVietnamese.
 */
/**
 * Place a position-level Stop Loss plan order. Bitget treats this as
 * "1 SL per (symbol, holdSide)" — calling again replaces the existing
 * SL trigger, so this is the right tool for the "kéo SL về entry"
 * break-even feature.
 *
 * triggerType:
 *   "mark_price" → trigger off Bitget's mark price (recommended; protects
 *                  against last-price wicks).
 *   "fill_price" → trigger off last traded price (more sensitive to wicks).
 */
export async function placePositionSL(
  creds: BitgetCreds,
  args: {
    symbol: string;
    /**
     * Bitget's convention DIFFERS by position mode (error 43011 otherwise):
     *   one-way mode → "buy" (long position) | "sell" (short position)
     *   hedge mode   → "long" | "short"
     * Callers must map from the account's posMode; use
     * holdSideForTpsl() below.
     */
    holdSide: "long" | "short" | "buy" | "sell";
    triggerPrice: string;
    triggerType?: "mark_price" | "fill_price";
    /** pos_loss = position SL (default), pos_profit = position TP. */
    planType?: "pos_loss" | "pos_profit";
    /** If omitted, Bitget uses the full open position. */
    size?: string;
  ...
  const body: Record<string, unknown> = {
    ...
    productType: "USDT-FUTURES",
    marginCoin: "USDT",
    planType: args.planType ?? "pos_loss",
    triggerPrice: args.triggerPrice,
    triggerType: args.triggerType ?? "mark_price",
    holdSide: args.holdSide,
    executePrice: "0", // "0" = market order on trigger (close at market)
  };
  if (args.size) body.size = args.size;
  if (args.clientOid) body.clientOid = args.clientOid;

  return signedPost<{ orderId: string; clientOid?: string }>(
    creds,
...
/**
 * Map a BrokerOrder's stored (side, posMode) to the holdSide string the
 * TPSL/close endpoints expect. Bitget uses buy/sell in one-way mode and
 * long/short in hedge mode — sending the wrong vocabulary returns 43011.
 */
export function holdSideForTpsl(
  orderSide: string,
  posMode: string,
): "long" | "short" | "buy" | "sell" {
  const isBuy = orderSide === "buy";
  if (posMode === "hedge_mode") return isBuy ? "long" : "short";
  return isBuy ? "buy" : "sell";
}

/**
 * Flash-close an OPEN position at market. Bitget's flash-close endpoint
 * closes the full position for (symbol, holdSide) immediately at market
 * price. holdSide is REQUIRED only in hedge mode; in one-way mode it must
 * be omitted (the symbol has at most one position).
 */
export async function flashClosePosition(
  creds: BitgetCreds,
  args: { symbol: string; holdSide?: "long" | "short" },
): Promise<{ orderId?: string } | null> {
  const body: Record<string, unknown> = {
    symbol: args.symbol,
    productType: "USDT-FUTURES",
  };
  if (args.holdSide) body.holdSide = args.holdSide;
  const data = await signedPost<{
    successList?: Array<{ orderId: string }>;
    failureList?: Array<{ orderId?: string; errorMsg?: string }>;
  }>(creds, "/api/v2/mix/order/close-positions", body);
  const first = data.successList?.[0];
  return first ? { orderId: first.orderId } : null;
}

/**
 * Cancel a single futures order.
 * Pass either orderId OR clientOid (orderId is preferred — clientOid only
 * works inside Bitget's active-orderbook window).
 *
 * Throws BitgetError with code 40768 if the order doesn't exist (already
 * filled, cancelled, or aged out) — caller should treat as success.
 */
export async function cancelOrder(
  creds: BitgetCreds,
  args: { symbol: string; orderId?: string; clientOid?: string },
): Promise<void> {
  if (!args.orderId && !args.clientOid) {
    throw new BitgetError("VALIDATION", "Cần orderId hoặc clientOid để huỷ.");
  }
  const body: Record<string, unknown> = {
    symbol: args.symbol,
    productType: "USDT-FUTURES",
  ...
  if (args.clientOid) body.clientOid = args.clientOid;
  await signedPost(creds, "/api/v2/mix/order/cancel-order", body);
}
...
export async function setPositionMode(
  ...
  await signedPost(creds, "/api/v2/mix/account/set-position-mode", {
    productType: "USDT-FUTURES",
    posMode: mode,
  });
}
...
export async function setLeverage(
  ...
  await signedPost(creds, "/api/v2/mix/account/set-leverage", body);
}
...
export async function placeOrder(
...
): Promise<{ raw: unknown; result: PlaceOrderResponse }> {
  const body: Record<string, unknown> = {
    symbol: input.symbol,
    productType: input.productType,
  ...
  if (input.orderType === "limit") {
    if (!input.price) {
      throw new BitgetError("VALIDATION", "Lệnh limit cần có giá.");
    }
    body.price = input.price;
    body.force = input.force ?? "gtc";
  }
  if (input.presetStopLossPrice) {
    body.presetStopLossPrice = input.presetStopLossPrice;
  }
  if (input.presetStopSurplusPrice) {
    body.presetStopSurplusPrice = input.presetStopSurplusPrice;
  }

  const data = await signedPost<{ orderId: string; clientOid: string }>(
    creds,
    "/api/v2/mix/order/place-order",
    body,
  );
  return { raw: data, result: data };
}
